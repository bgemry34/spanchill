...
if(isset($_POST['editService'])){
  ...
  if(file_exists($_FILES['image']['tmp_name']) || is_uploaded_file($_FILES['image']['tmp_name'])){
    ...
    if(in_array($fileActualExt, $allowed)){
        if($fileError===0){
            if($fileSize<2*1048576){
              ...
              $stmt = $mysqli->prepare("update services set name=?, price=? image=? where id = ?");
              $stmt->bind_param("ssss", $name, $price, $fileNameNew, $id);
              if ($stmt->execute()) {
                  $stmt->close();
                  echo "<script>alert('Edited Successfully');window.location.replace('services.php')</script>";
              } else {
    ...
    $id = $_POST['id'];
    $mysqli = new mysqli('localhost', 'root', '', 'spanchill');
    $stmt = $mysqli->prepare("update services set name=?, price=? where id = ?");
...
?>
        ...
        ?>
            ...
            <div class="form-group">
                ...
                <input type="number" class="form-control" name="price" placeholder="" value="<?=$row['price']?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <input type="submit" value="Submit" class="form-control btn btn-success" name="editService" id="">
            </div>
        </form>
        </div>
    </div>
